...
/* ── Hero — same purple gradient as original roadmap ── */
.rm-hero {
...
.rm-top-row { display: flex; align-items: center; gap: 10px; margin-bottom: 20px; }
.rm-back {
    display: inline-flex; align-items: center; gap: 6px;
    color: rgba(255,255,255,.85) !important; font-size: 13px; font-weight: 600;
    text-decoration: none !important; padding: 7px 16px; border-radius: 30px;
    border: 1px solid rgba(255,255,255,.3); transition: all .2s;
}
.rm-back:hover { color: #fff !important; background: rgba(255,255,255,.15); border-color: rgba(255,255,255,.55); }
.rm-chip {
    display: inline-flex; align-items: center; gap: 6px;
...
.rm-hero h1 {
    ...
    color: #fff !important; margin: 0 0 8px; letter-spacing: -.3px;
}
.rm-hero p { color: rgba(255,255,255,.72) !important; font-size: 14px; margin: 0; }
...
.rm-card:nth-child(n+5) { animation-delay: .25s; }
.rm-card:hover {
...
.rm-card-body { padding: 22px 20px 14px; flex: 1; }
...
.rm-title { font-size: 17px; font-weight: 700; color: #1a1a2e; margin-bottom: 6px; line-height: 1.3; }
.rm-desc  { font-size: 13px; color: #8a8fad; line-height: 1.55; margin-bottom: 14px; }
.rm-stats { display: flex; gap: 14px; flex-wrap: wrap; }
.rm-stat  { font-size: 12px; color: #8a8fad; }
.rm-stat.is-done { color: #00c48c; font-weight: 600; }
...
.rm-pw-pct { font-weight: 700; color: #1a1a2e; }
.rm-pw-track { height: 4px; background: #e4e6f0; border-radius: 99px; overflow: hidden; }
.rm-pw-fill { height: 100%; background: linear-gradient(90deg, #667eea, #764ba2); border-radius: 99px; transition: width .6s ease; }
.rm-pw-fill.full { background: linear-gradient(90deg, #00c48c, #00e5a8); }

/* Start button — same style as "+ Add Skill" */
.rm-go {
    ...
    transition: all .2s; box-shadow: 0 4px 14px rgba(102,126,234,.3);
}
.rm-go:hover {
...
@php use App\Models\UserTaskProgress; @endphp
...
<div class="rm-wrap">
    ...
    <div class="container">
        <div class="rm-count">
            ...
            <span class="rm-count-badge">{{ $roadmaps->count() }} paths</span>
        </div>
            ...
            <div class="rm-card">
                ...
                <div class="rm-card-body">
                    ...
                    <div class="rm-stats">
                        ...
                        @endif
                        @if($done > 0)
                            <span class="rm-stat is-done">✔ {{ $done }} done</span>
                        @endif
                    </div>
                </div>
                <div class="rm-footer">
